let buttonf=document.querySelector('#new-quote');
 let quotef=document.querySelector('.quote');
 let authorf=document.querySelector('.author');
 let professionf=document.querySelector('.profession');
 let topic=document.querySelector('.topic');
 let topicsList=document.querySelector('.topic-list');
...
buttonf.addEventListener('click', function(){
    ...
    quotef.innerText=quotesArray[rand].quote;
    authorf.innerText=quotesArray[rand].author;
    professionf.innerText=quotesArray[rand].profession;
    // topic.innerText=quotesArray[rand].topics;

    // for (i=0; i<quotesArray.length; i++){
    //     for (a=0; quotesArray[a].topics)
    ...
    const topicArray=quotesArray[rand].topics;
    if (topicsList.childElementCount>0){
        topicsList.replaceChildren();
    }
    
    for (let i=0; i<topicArray.length; i++){
        var li = document.createElement('li')
        li.innerText +=topicArray[i];
        topicsList.appendChild(li)
        
        // topicsList.appendChild(li);
        // if (topicsList.childElementCount=== (topicArray.length+1)){
        //     topicsList.replaceChildren;
        //  }
    
    }
})
// console.log(topicsList.childElementCount);